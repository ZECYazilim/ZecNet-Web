...
public class BankaHesapAppService : OnMuhasebeAppService, IBankaHesapAppService
{
    ...
    public virtual async Task<PagedResultDto<ListBankaHesapDto>> GetListAsync(BankaHesapListParameterDto input)
    {
        ...
        mappedDtos.ForEach(x =>
        {
            ...
            x.Borc = x.MakbuzHareketler
            .Where(y => y.BelgeDurumu == Makbuzlar.BelgeDurumu.TahsilEdildi || 
                   y.OdemeTuru == Makbuzlar.OdemeTuru.Pos && y.BelgeDurumu == Makbuzlar.BelgeDurumu.Portfoyde)
                   .Sum(y=>y.Tutar);

            x.Alacak = x.MakbuzHareketler.Where(y => y.BelgeDurumu == Makbuzlar.BelgeDurumu.TahsilEdildi)
                     .Sum(y => y.Tutar);
        });
    ...
    public virtual async Task<SelectBankaHesapDto> CreateAsync(CreateBankaHesapDto input)
    {
        await _bankaHesapManager.CheckCreateAsync(input.Kod, input.BankaSubeId
            , input.OzelKod1Id, input.OzelKod2Id, input.SubeId); //kontroller yapılıyor.
    ...
    public virtual async Task<SelectBankaHesapDto> UpdateAsync(Guid id, UpdateBankaHesapDto input)
    {
        ...
        await _bankaHesapManager
            .CheckUpdateAsync(id, input.Kod, entity, input.BankaSubeId, input.OzelKod1Id, input.OzelKod2Id);
