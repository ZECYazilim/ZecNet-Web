...
public class BankaSubeManager : DomainService
{
    ...
    public async Task CheckCreateAsync(string kod, Guid? bankaId,Guid? ozelKod1Id, Guid? ozelKod2Id)
    {
        ...
        await _bankaSubeRepository.CodeAnyAsync(kod, x => x.Kod == kod && x.BankaId == bankaId);
    ...
    {
        await _bankaSubeRepository.CodeAnyAsync(kod,
            x => x.Id != id && x.Kod == kod && x.BankaId == entity.BankaId,
            entity.Kod != kod);
    ...
    {
        await _bankaSubeRepository.RelationtalEntityAnyAsync(
