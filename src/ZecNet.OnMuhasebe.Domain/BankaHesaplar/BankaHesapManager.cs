...
public class BankaHesapManager:DomainService
{
    ...
    public async Task CheckCreateAsync(string kod,Guid? bankaSubeId, Guid? ozelKod1Id, Guid? ozelKod2Id,Guid? subeId)
    {
        ...
        await _bankaHesapRepository.CodeAnyAsync(kod, x => x.Kod == kod && x.SubeId == subeId);
    ...
    {
        await _bankaHesapRepository.
            CodeAnyAsync(kod, x => x.Id != id && x.Kod == kod && x.SubeId == entity.SubeId,entity.Kod!=kod);

        await _bankaSubeRepository.
            EntityAnyAsync(bankaSubeId, x => x.Id == bankaSubeId, entity.BankaSubeId != bankaSubeId);

        await _ozelKodRepository.
            EntityAnyAsync(ozelKod1Id, OzelKodTuru.OzelKod1, KartTuru.BankaHesap, entity.OzelKod1Id != ozelKod1Id);

        await _ozelKodRepository.
            EntityAnyAsync(ozelKod2Id, OzelKodTuru.OzelKod2, KartTuru.BankaHesap, entity.OzelKod2Id != ozelKod2Id);
    }
    public async Task CheckDeleteAsync(Guid id)
    {
        await _bankaHesapRepository.RelationtalEntityAnyAsync(
